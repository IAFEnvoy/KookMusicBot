...
class Player:
    ...
    def add_music(self, music: str, extra_data: dict = None):
        ...
        if self.token is None:
            raise ValueError('第一次启动推流时，你需要指定机器人token')
        if self.guild_id not in play_list:
            play_list[self.guild_id] = {'token': self.token,'now_playing': None,'play_list': [],'extra_data':[]}
        ...
        play_list[self.guild_id]['extra_data'].append(extra_data)
    ...
    def list(self):
        global now_play_extra_data
        if self.guild_id not in play_list:
            raise ValueError('该服务器没有正在播放的歌曲')
        return [play_list[self.guild_id]['now_playing'],now_play_extra_data, play_list[self.guild_id]['extra_data']]
        ...
        if True:
            ...
            while True:
                ...
                if play_list[self.guild]['now_playing'] and not play_list[self.guild]['play_list']:
                    ...
                    global now_play_extra_data
                    now_play_extra_data=play_list[self.guild]['extra_data'].pop(0)
